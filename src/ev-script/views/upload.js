define(function(require) {
    ...
    return BaseView.extend({
        ...
        decorateUploader: function() {
            ...
            mediaTypesCollection.on('reset', _.bind(function() {

                this.$('.policy-message').html(policyMessage);

                extensions = mediaTypesCollection.pluck('extension');
                extensions = _.map(extensions, function(extension) {
                    return extension.replace(/^\*\./g, '').replace(/^\./g, '');
                });
                ...
                this.$upload.pluploadQueue({
                    url: this.config.ensembleUrl + '/app/unprotected/uploads.ashx?publishContent=true',
                    runtimes: 'html5,html4,flash',
                    max_file_size: maxUploadSize + 'gb',
                    max_file_count: 1,
                    max_retries: 5,
                    chunk_size: '2mb',
                    unique_names: false,
