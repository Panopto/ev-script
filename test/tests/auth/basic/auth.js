define(function(require) {
    ...
    if (!evSettings.authType || evSettings.authType === 'basic') {
        ...
        q.test('check api', 5, function() {
            q.ok(_.isFunction(this.auth.getUserId), 'expected getUserId');
        ...
        q.test('login/logout test', 6, function() {
            var username = 'foo',
                password = 'bar';
            var loginPromise = this.auth.login({
                username: username,
                password: password
            });
            // We expect the promise to be resolved
            q.ok(loginPromise.state() === 'resolved');
            q.ok(this.auth.isAuthenticated());
            q.strictEqual(username, this.auth.getUserId());
            var logoutPromise = this.auth.logout();
            q.ok(logoutPromise.state() === 'resolved');
            q.ok(!this.auth.isAuthenticated());
            q.strictEqual(null, this.auth.getUserId());
        });
        ...
        q.test('domain test', 2, function() {
            ...
            q.ok(this.auth.isAuthenticated());
            q.strictEqual(username + '@' + this.auth.config.authDomain, this.auth.getUserId());
        });
